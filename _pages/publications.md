---
layout: archive
title: "Publications"
permalink: /publications/
author_profile: true
---
---
layout: archive
title: "Publications"
permalink: /publications/
author_profile: true
---

<sup>\*</sup> denotes equal contribution. See also my [Google Scholar profile](https://scholar.google.com/citations?user=Ifht6gwAAAAJ&hl=en).

## Peer-Reviewed Publications


**Classifying Metamorphic versus Single-Fold Proteins with Statistical Learning and AlphaFold2**
**Yongkai Chen**, Samuel W. K. Wong, S. C. Kou
*Annals of Applied Statistics*, accepted.
[arXiv](https://arxiv.org/abs/2512.10066)

**Wahkon: A Statistically Principled Deep RKHS Superposition Network**
**Yongkai Chen**, Wenxuan Zhong, Ping Ma
*[STAI-X 2026 Conference](https://arxiv.org/abs/2605.14041)*. **Best Paper Award.**
 ·

**Graphon Cross-Validation: Assessing Models on Network Data**
Huimin Cheng\*, **Yongkai Chen**\*, Ping Ma, Wenxuan Zhong
*International Conference on Learning Representations (ICLR)*.

**SpaDiff: Denoising for Sequence-based Spatial Transcriptomics via Diffusion Process**
Jiazhang Cai, Yongkai Chen, Luyang Fang, Wenxuan Zhong, Guo-Cheng Yuan, Ping Ma
*Cell Reports Methods*.
[bioRxiv](https://doi.org/10.1101/2025.10.07.681011)

**Knowledge Cascade: Reverse Knowledge Distillation on Nonparametric Multivariate Functional Estimation**
Luyang Fang, Haoran Lu, **Yongkai Chen**, Wenxuan Zhong, Ping Ma
*Journal of Machine Learning Research*, 27(54):1–38.

**Network Perturbation Aggregation for Graphon Estimation**
Da Wang, Jun S. Liu, **Yongkai Chen**, Yuyao Wang, Huimin Cheng
*Statistical Learning and Data Science*, 100005.

**Knowledge Distillation and Dataset Distillation of Large Language Models: Emerging Trends, Challenges, and Future Directions**
Luyang Fang, Xiaowei Yu, Jiazhang Cai, **Yongkai Chen**, Shushan Wu, Zhengliang Liu, Zhenyuan Yang, Haoran Lu, Xilin Gong, Yufang Liu, et al.
*Artificial Intelligence Review*, 59(1):17.



**Fluid Correlation: A Novel Nonparametric Metric to Assess the Dynamic Association**
**Yongkai Chen**, Shushan Wu, Ping Ma, Wenxuan Zhong
*Journal of Computational and Graphical Statistics*, 34(4):1331–1343.

**A Machine Learning Model Using Echocardiographic Myocardial Strain to Detect Myocardial Ischemia**
Bo Zheng, Yaokun Liu, Jingyi Zhang, Terry T. Ma, Yun Zhou, **Yongkai Chen**, Ying Yang, Wei Ma, Fangfang Fan, Jia Jia, et al.
*Internal and Emergency Medicine*, 20(5):1425–1436.



**Bisection Grover's Search Algorithm and Its Application in Analyzing CITE-seq Data**
Ping Ma\*, **Yongkai Chen**\*, Haoran Lu, Wenxuan Zhong
*Journal of the American Statistical Association*, Applications and Case Studies, 1–12.
[code](https://github.com/StatCYK/BGS-for-CITEseq)

**Bayesian Knowledge Distillation: A Bayesian Perspective of Distillation with Uncertainty Quantification**
Luyang Fang\*, **Yongkai Chen**\*, Wenxuan Zhong, Ping Ma
*Forty-first International Conference on Machine Learning (ICML)*.

**Orthogonal Multimodality Integration and Clustering in Multimodal Single-cell Data**
**Yongkai Chen**\*, Yufang Liu\*, Haoran Lu, Wenxuan Zhong, Guocheng Yuan, Ping Ma
*BMC Bioinformatics*.



**Elucidating Dynamic Cell Lineages and Gene Networks in Time-Course Single Cell Differentiation**
Mengrui Zhang\*, **Yongkai Chen**\*, Dingyi Yu, Wenxuan Zhong, Jingyi Zhang, Ping Ma
*Artificial Intelligence in the Life Sciences*, 3:100068.



**Asymptotic Analysis of Sampling Estimators for Randomized Numerical Linear Algebra Algorithms**
Ping Ma, **Yongkai Chen**, Xinlian Zhang, Xin Xing, Jingyi Ma, Michael W. Mahoney
*Journal of Machine Learning Research*, 23:1–45.

**Smoothing Splines Approximation Using Hilbert Curve Basis Selection**
Cheng Meng, Jun Yu, **Yongkai Chen**, Wenxuan Zhong, Ping Ma
*Journal of Computational and Graphical Statistics*, 31(3):802–812.



**Ensemble Machine Learning Approach for Screening of Coronary Heart Disease Based on Echocardiography and Risk Factors**
Jingyi Zhang, Huolan Zhu, **Yongkai Chen**, Chenguang Yang, Huimin Cheng, Yi Li, Wenxuan Zhong, Fang Wang
*BMC Medical Informatics and Decision Making*, 21(1):187.

**Drug Screening with Zebrafish Visual Behavior Identifies Carvedilol as a Potential Treatment for an Autosomal Dominant Form of Retinitis Pigmentosa**
Logan Ganzen, Mee Jung Ko, Mengrui Zhang, Rui Xie, **Yongkai Chen**, Liyun Zhang, Rebecca James, Jeff Mumm, Richard M. van Rijn, Wenxuan Zhong, et al.
*Scientific Reports*, 11(1):11432.

## Preprints and Manuscripts Under Review

**Uncovering Distinct Protein Conformations Using Coevolutionary Information and AlphaFold**
**Yongkai Chen**, Samuel W. K. Wong, S. C. Kou
[bioRxiv](https://doi.org/10.1101/2025.10.08.681198) · [code](https://github.com/StatCYK/SMICE)

**Quantum Statistical Bootstrap**
**Yongkai Chen**, Ping Ma, Wenxuan Zhong
[arXiv](https://arxiv.org/abs/2604.00951) · [code](https://github.com/StatCYK/quantum-bootstrap)

**Multi-Teacher Knowledge Distillation via Teacher-Informed Mixture Priors**
Luyang Fang, **Yongkai Chen**, Jiazhang Cai, Ping Ma, Wenxuan Zhong
[arXiv](https://arxiv.org/abs/2605.27967)

**S2MNet: Speckle-To-Mesh Net for Three-Dimensional Cardiac Morphology Reconstruction via Echocardiogram**
Xilin Gong, **Yongkai Chen**, Shushan Wu, Fang Wang, Ping Ma, Wenxuan Zhong
[arXiv](https://arxiv.org/abs/2505.06105)

**Alignment and Safety in Large Language Models: Safety Mechanisms, Training Paradigms, and Emerging Challenges**
Haoran Lu, Luyang Fang, Ruidong Zhang, Xinliang Li, Jiazhang Cai, Huimin Cheng, Lin Tang, Ziyu Liu, Zeliang Sun, Tao Wang, et al.
[arXiv](https://arxiv.org/abs/2507.19672)

**Decreased GABA Levels During Development Result in Increased Connectivity in the Larval Zebrafish Tectum**
Yang Liu\*, **Yongkai Chen**\*, Carly R. Duffy\*, Ariel J. VanLeuven, John Branson Byers, Hannah C. Schriever, Rebecca E. Ball, Jessica M. Carpenter, Chelsea E. Gunderson, Nikolay M. Filipov, et al.
[bioRxiv](https://doi.org/10.1101/2024.09.11.612511) · [code](https://github.com/StatCYK/neural-activity-analysis)
